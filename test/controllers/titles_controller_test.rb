...
class TitlesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create title' do
    assert_difference('Title.count') do
      post titles_url,
           params: { title: { content_url: @title.content_url, desccription: @title.desccription, name: @title.name,
                              thumbnail_url: @title.thumbnail_url, user_id: @title.user_id, year: @title.year } }
    end
